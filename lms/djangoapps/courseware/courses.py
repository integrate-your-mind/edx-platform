...
_Assignment = namedtuple(
    'Assignment', ['title', 'url', 'date', 'requires_full_access', 'complete', 'past_due']
)
...
def get_course_assignments(course_key, user, request, include_access=False):
    ...
    for section in course_root_block.get('children', []):
        for subsection in section.get('children', []):
            ...
            requires_full_access = include_access and _requires_full_access(subsection, user)
            ...
            assignments.append(
                _Assignment(title, url, subsection.get('due'), requires_full_access, complete, past_due)
            )

    return assignments


def _requires_full_access(block, user):
    """
    Returns a boolean if any children of the block specified has a group_access array consisting of just full_access
    """
    for child in block.get('children', []):
        group_access = child.get('group_access')
